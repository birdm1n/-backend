...
public class GoodsMgmtController {
    ...
    @GetMapping("/goodsMatchBarcode/{barcode}")
    public ResponseEntity<CommonResponse<ResponseDto<GoodsMatchRespDto>>> goodsMatchBarcode(@ApiParam(value = "바코드", required = true) @PathVariable String barcode) {

        return responseHandler.getResponseMessageAsRetrieveResult(goodsMgmtService.goodsMatchBarcode(barcode), ResponseCodeEnum.NODATA.getResultCode(), ResponseCodeEnum.NODATA.getResultMsg());
    }
}
